...
def ask_json(prompt: str) -> dict | None:
    """Pede JSON em linha. Mostra exemplo antes de pedir."""
    console.print('[dim]ex: {"campo": "valor", "num": 42}[/dim]')
    raw = ask(prompt)
    if not raw:
        return None
    try:
        ...
        console.print("[fail]JSON inválido — use aspas duplas[/fail]")
        return None


def ask_json_interactive(con: "Console | None" = None) -> dict | None:
    """
    Constrói payload campo a campo — sem precisar escrever JSON na mão.
    Termina quando o campo ficar vazio.
    """
    c = con or console
    ...
    while True:
        ...
        try:
            payload[field] = json.loads(raw_val)   # infere int, bool, null
        except Exception:
            payload[field] = raw_val               # fallback: string
    if not payload:
        c.print("[fail]payload vazio[/fail]")
        return None
    c.print(f"\n[dim]payload:[/dim] [info]{json.dumps(payload)}[/info]")
    return payload
